...
# 2026年
## 会話サポートサービス「Ally」 (ハッカソン)
#React #ハッカソン

![[/assets/products/gdgoc/gdgoc-hackathon-3-features.png]]

「Ally」は議論を円滑にするための会話サポートアプリです．
主な機能に議論を図に表して，整理する「ダイアログマッピング」，用語をコンテキストに合わせて解説してくれる「用語解説機能」，会話内容を文字起こしする「リアルタイム文字起こし機能」があります．

**4人チームで参加し，函館会場でnoolab賞を受賞しました．**

[[会話サポートサービス「Ally」（GDGoC Japan Hackathon）|詳細はこちら]]

## WebAssemblyのチェックポイント・レストア機構のマルチスレッド対応
#WebAssembly #C | [Githubリポジトリ](https://github.com/tauto1127/wasm-micro-runtime/tree/dev/migration-threaded-wasm)

![[/assets/products/research/wasm-checkpoint-restore.png]]

WebAssemblyアプリケーションを対象としたチェックポイント・レストア機構の研究をしています．

チェックポイント・レストア機構は，アプリケーションが持つ状態をファイルに書き出し，そのファイルを移行先のデバイスで読み取ることで，実行中のアプリケーションを移行することが可能になります．
ユースケースとして，エッジサーバーのハンドオーバー時のライブマイグレーションなどがあります．

WebAssemblyを用いることで，X86やARMなどのアーキテクチャに依存しないライブマイグレーションが可能になります．
学部では，単一処理を前提としていたチェックポイント・レストア機構を並列処理環境へと拡張を行いました．

並列処理環境への拡張にあたっては，WebAssemblyを動かすWasmランタイムにてどのようにスレッド処理が実現されているかを解読し，機構の設計を行いました．
実際に実装，評価まで行っており，ネットワークには対応していないという制約はあるものの，熱拡散シミュレーションなどの並列処理を必要とするアプリケーションに対して，ライブマイグレーションが可能であることを示すことができました．

大学院では，Robot Operating Systemを対象に，ライブマイグレーションを適用するための拡張を行いたいと思っています．
